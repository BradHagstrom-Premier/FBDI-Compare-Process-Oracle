# FBDI ↔ Applaud Mapping Audit — Design

**Date:** 2026-04-21
**Status:** Design approved by Brad; ready for implementation plan
  ...
  "tables": [
    {
      ...
      "fields": [
        { "name": "TA4INTERFACE_LINE_ATTRIBUTE1", "bare_name": "INTERFACE_LINE_ATTRIBUTE1", "data_type": "...", "length": 30 }
      ]
    }
  ],
  "missing_tables": [
...
2. **Schema probe.** Before the loop, query DataDictionary for one known-good prefix (e.g. `TA4`) to confirm the column names used for field name, data type, and length. Abort loudly if the probe fails.
3. **Per-table loop** (all 183):
   - `get_table_definition(name)` → description, prefix (parsed from the "(<prefix>)" suffix in the description), type, key sequences.
   - `execute_query("SELECT … FROM DataDictionary WHERE DDID1 = '<prefix>' …")` → fields for that prefix. Each field gets a `bare_name` with the prefix stripped.
...
| `column_overlap` | Fraction of the Applaud table's DataDictionary fields (bare names) present in that FBDI tab's column set. `0.0` to `1.0`. |
...
- Applaud table with no DataDictionary fields → `column_overlap` is N/A; `key_coverage` alone drives candidacy.
...
4. MULTI prior claims (31 rows currently):
   ...
   - KEEP MULTI with NOTE if: every claim scores High-or-Medium, but at least one is Medium (note reminds Brad that one leg is PARTIAL-name-aligned)
   - COLLAPSE to highest-scoring if: only one claim scores High/Medium, others Low or absent from candidates
   ...
   Note: a shared technical tab name is a common pattern (e.g. `T_EGP_COMPONENTS_INTERFACE` mapped to `ChangeOrderImportTemplate / EGP_COMPONENTS_INTERFACE` and `ItemStructureImportTemplate / EGP_COMPONENTS_INTERFACE`) but is not a precondition — some legitimate multi-mappings span differently-named tabs (e.g. `T_AWARD_KEYWORDS` spans `Award Keywords` and `Grants Personnel Keywords`). The rubric's column-overlap and key-coverage signals carry the weight, not tab-name identity.
...
2. **Pass 1 thresholding** — candidate kept/dropped by thresholds; sort order.
3. **Prior-mapping parser** — single, multi, blank, malformed input.
4. **Adjudication branches** — every path in §6.1 algorithm gets its own test:
...
5. **Prefix conformance** — matching / mismatch with note.
6. **Output writer** — three sheets, correct headers, NEEDS_REVIEW content on Sheet 3.

### 8.2 Integration test

`test_audit_end_to_end`:
- Synthetic snapshot with 5 Applaud tables covering EXACT / PARTIAL / UNMAPPED / multi-collapse / needs-review.
...
**Target:** ~25-35 new tests. Full suite goes from 139 → ~170. Runtime under 30s.
