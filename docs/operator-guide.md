...
Every quarter, Oracle ships a new release of its FBDI (File-Based Data Import) template files. This pipeline compares the new release against the previous one, field by field, across all templates, and produces two deliverables: a comparison report showing exactly what changed, and an updated master catalog of every field in every template for the new release. Definian team members who maintain Oracle integrations use these outputs to keep those integrations aligned with Oracle's changes. This guide is for whoever is running that quarterly refresh. No Python knowledge required.

---

## Before your first run

**Environment.** You need Python 3.14 or higher and Google Chrome installed on your machine. This pipeline runs on Windows. Before your first run, open a terminal and run:

```
pip install -r requirements.txt
```

That installs the handful of Python packages the pipeline depends on.

**Where files land.** Downloads go into a folder called `baselines/<release>/originals/`, like `baselines/26B/originals/`. These folders aren't tracked by Git (they're gitignored), so they won't interfere with source control. If you use the Claude Code skill to run the pipeline, it creates these folders for you. If you run the raw CLI commands manually, you'll need to create the folder yourself before starting.
...
**What it does:** Checks that your machine has everything the pipeline needs before doing any real work. It verifies Python version, Chrome presence, and that required packages are installed.

**What you see on screen:** A short pass/fail report. Green means proceed. It'll tell you exactly what's wrong if something fails.
...
**What it does:** Figures out which release you're comparing from and which you're comparing to. If you told the skill explicitly (e.g., "compare 26A to 26B"), it uses those. Otherwise it looks at what's already in `baselines/` and infers the previous release, then infers the new release as the next quarter. The naming convention after 26D is 27A.
...
**What it does:** Downloads all FBDI template files from Oracle's documentation site for the new release (and for the old release too, if it's not already on disk). It uses browser automation to scrape and download each file, then verifies the download against a known inventory. If files are missing after the first attempt, it retries once automatically.

**What you see on screen:** Download progress as files are fetched, then a verification summary showing how many files were expected, how many were found, and any gaps. Expect to see filenames scrolling by for 15–20 minutes.

**Expected wall time:** 15–20 minutes per release being downloaded.

**If it stalls:** If the browser freezes, kill the process and re-invoke the skill. It will re-download. Note that re-running Stage 3 wipes `originals/` first, so don't restart it partway through if you're trying to preserve a partial download.

---

### Stage 4 — Smart-clear

**What it does:** Creates "blank" copies of the new release's templates in `baselines/<NEW>/blanks/`. Blank means headers are preserved but all data rows are removed. These are the clean template files you'd hand to a client. The clearing logic is smart enough to handle headers at different row positions across different templates.
...
**If it stalls:** The overall stage won't stall. Individual file timeouts are handled gracefully and don't block the rest. If the whole stage hangs, kill and re-run. It's safe to repeat.
...
**If it stalls:** It shouldn't. If it errors, the comparison report and catalog are unaffected. This stage is read-only.

---

## The 6 HITL checkpoints

The pipeline has six human-in-the-loop checkpoints where it stops and asks you what to do. These are labeled HITL #1 through #6, which are stable IDs from the design spec, not the order they appear during a run. HITL #3, for instance, fires before HITL #1 because version resolution happens before baseline presence is checked.
...
**How to decide:** If you've run a prior quarter's refresh and the `baselines/` folder is intact, confirm the path. If it's genuinely missing, download it. That adds another 15–20 minutes but is the clean path.

---

### HITL #2 — `RapidImplementationForCashManagement.xlsm` missing

**Trigger:** After the download completes, this specific file isn't present in `baselines/<NEW>/originals/`. It's a bank account setup template that Oracle doesn't host on the standard FBDI documentation pages, so the downloader never finds it.

**Options presented:**
- Copy it from the prior release's folder (fast, safe, since Oracle rarely updates this template)
...
**How to decide:** Option (a), copying from the prior release, is the right call in almost every case. Only go to Oracle Fusion if you have a specific reason to believe this template changed in the new release.
...
**Options presented:**
- Confirm you want to skip releases (unusual, but proceed as asked)
- Correct to the detected version

**How to decide:** Skipping releases is rarely intentional. If you're catching up after a missed quarter, confirm explicitly. If this prompt was unexpected, it probably means you mistyped a version number.
...
**Trigger:** After the automatic retry, there are still files in the expected inventory that weren't downloaded.
...
**Options presented:**
- Add the extra files to the inventory (the right call if Oracle added new templates)
- Quarantine them to a separate folder and keep them out of the comparison
- Review them file by file before deciding

**How to decide:** Option (a) is almost always correct. Oracle does add new templates in quarterly releases. Quarantine is only appropriate if you suspect the scraper accidentally pulled something from the wrong page, which is rare.

---

## Reading the outputs

You get two output files after a successful run, both in the repo root.

**`Comparison_Report_<OLD>_<NEW>.xlsx`** is the change log. It has seven columns: File, Tab, Position, Label, Technical, Change Type, and Details. Change Type is one of Added, Removed, or Modified. Each row represents a single field-level change in a single template tab. This is the file you hand to whoever owns the Applaud mapping. They use it to identify which integrations need updating before the new release goes live. When you're triaging, sort by File or Tab to group changes by area; sort by Change Type to pull out all Removed fields first (those are highest risk for existing integrations).

**`FBDI_Master_Catalog.xlsx`** is the full snapshot. It has three kinds of sheets:

- Per-release tabs (e.g., `26A`, `26B`) with one row per file × tab × column, including position, label, technical name, data type, length, scale, and required flag
- An `Issues` sheet flagging fields where Oracle's type strings are genuinely malformed (currently 9 rows, all known Oracle data quality issues, not a sign that something went wrong in your run)
- A `Drift` sheet tracking tabs that moved between files across releases
