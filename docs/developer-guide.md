...
Phase 2 of the repo cleanse (PR #2) is done: the release change report now carries the Definian design system (Tier B) — a navy branded cover, module→file/tab structure, color-coded change tables, and self-contained inlined Aptos fonts — HTML-first with an opt-in PDF. The data layer was untouched; it was a presentation change. Design at `docs/superpowers/specs/2026-09-14-repo-cleanse-and-report-redesign-design.md` (§6); the `fbdi/report.py` section below covers the template architecture.
...
**`fbdi/report.py`** generates the release change report. `generate_report(catalog_path, old_release, new_release, out_dir, pdf=False) -> (html_path, pdf_path|None)`. It reads the master catalog's per-release sheets, aligns each `(file, tab)` across the two releases via `align.align_tabs`, and groups the changed tabs by Oracle module using each release's `baselines/<release>/file_modules.json` (NEW wins over OLD; files with no entry fall under "Unclassified"). It renders the Jinja2 template `fbdi/templates/report.html.j2` (change tables live in the imported `_report_macros.j2`). HTML is always written; the PDF is opt-in (`pdf=True`), and weasyprint is imported lazily only then — so the common HTML path has no heavy dependency. The PDF rendering needs MSYS2 mingw64 GTK on Windows; if `python -m fbdi report --pdf` blows up on Pango, that's the cause.

The template applies the **Definian design system** (Tier B) — see `DESIGN.md`. It renders twice: once for screen (`print_mode=False` — sticky search/filter toolbar and collapsible file sections via vanilla JS) and once for print (`print_mode=True` — toolbar/JS dropped, shift details expanded, `@page` set). Both renders receive a `design_css` variable that `generate_report` loads from the vendored, self-contained `fbdi/templates/assets/definian.css` (Definian tokens + 5 base64-inlined Aptos faces) and inlines with `| safe`, so the HTML is one portable file with no external CSS/font fetch and deterministic glyphs in weasyprint. Unit tests render the template *without* `design_css` (the template guards it), keeping them fast and the font blob out of assertions. Regenerate the CSS asset with `py tools/build_report_font_css.py` — it reads the design system's `fonts/*.ttf` and rewrites the vendored file; run it only when the embedded face set changes.
...
- `verify_download.py` checks the downloaded file count against `baseline_files.txt`. Exit 0 means clean, 1 means files are missing, 2 means extras only, 3 means first-run bootstrap (no entry for this release in the inventory). Its `--reconcile` mode is a no-download audit runnable on any invocation (the Stage 4.5 gate, resume, report-only): it asserts `baselines/<ver>/originals/` still holds every file its inventory reference expects (the release's own section when present, else the prior release), failing (exit 1) on any shortfall. The match is asymmetric — a file present last quarter but absent now is a hard fail; net additions are informational. This is the guard against silent baseline drift (a template that failed to download and was never noticed).
- `summarize_report.py` prints comparison statistics (changed, added, removed, total) from a completed report workbook.
- `verify_run.py` is a post-run health check. It runs diagnose, looks for regressions in the Issues tab versus the prior release, and reconciles `baselines/<ver>/originals/` against `baseline_files.txt`. Exit 0 means clean; exit 1 means a soft regression was flagged (diagnose / catalog Issues — non-blocking, surfaced as a warning); exit 2 means a baseline inventory hard-fail (the originals folder is short vs its reference — blocking).
